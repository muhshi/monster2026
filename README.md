...
- **Changed (16 Juli 2026)**: Menaikkan `size` payload API dari `5` menjadi `10` (batas maksimal API FASIH) pada `monitoring_lapangan.py` dan `fetch_pengawas.py` untuk mengurangi jumlah request halaman dan meminimalisir risiko data terlewat akibat pagination shifting.
- **Fixed (16 Juli 2026)**: Mengubah assignment status (`=`) menjadi akumulator (`+=`) pada parsing `statusBreakdown` di `monitoring_lapangan.py` agar status dengan nama mirip (misalnya ada dua entry status dalam satu region) tidak saling overwrite dan terakumulasi dengan benar.
- **Added (16 Juli 2026)**: Menambahkan deteksi dan logging untuk status API yang tidak dikenali (⚠️ warning) beserta ringkasan per-halaman (📊 Open/Draft/Submitted/Approved/Rejected) untuk mempermudah diagnosis perbedaan data antara script dan dashboard FASIH.
